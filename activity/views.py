...
@login_required
@method_decorator(permission_required('activity.view_activitytype', raise_exception=True), name='dispatch')
...
@login_required
@method_decorator(permission_required('activity.add_activitytype', raise_exception=True), name='dispatch')
...
@login_required
@method_decorator(permission_required('activity.change_activitytype', raise_exception=True), name='dispatch')
...
@login_required
@method_decorator(permission_required('activity.delete_activitytype', raise_exception=True), name='dispatch')
...
class AddActivityViewCM(View):
    ...
    def post(self, request, subject_id):
        ...
        messages.success(request, "ℹ️ You will now be redirected to configure quiz types.")
...
def UpdateActivity(request, activity_id):
    ...
    if request.method == 'POST':
        ...
        if start_time and end_time and end_time <= start_time:
            ...
            return redirect('UpdateActivity', activity_id=activity_id)
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Activity update process completed.')
...
class EditParticipationView(View):
    def get(self, request, activity_id):
        ...
        activity = get_object_or_404(Activity, id=activity_id)
        return render(request, 'activity/question/editParticipation.html', {
            'activity': activity,
...
class UpdateQuestionViewCM(View):
    def get(self, request, activity_id, index):
        ...
        return render(request, 'activity/question/updateQuestionCM.html', {
            'activity_id': activity_id,
            'activity': activity,
            'index': index,
            'subject': subject,
...
class DisplayQuestionsView(View):
    def get(self, request, activity_id):
        ...
        is_time_keeper = user.is_authenticated and user.profile.role.name.lower() == 'time keeper'
...
class SubmitAnswersView(View):
    def post(self, request, activity_id, auto_submit=False):
        ...
        if activity.retake_method == 'highest':
            highest_score = RetakeRecord.objects.filter(student_activity=student_activity).aggregate(Max('score'))['score__max']
            student_activity.total_score = highest_score if highest_score is not None else 0

        elif activity.retake_method == 'latest':
            latest_record = RetakeRecord.objects.filter(student_activity=student_activity).order_by('-retake_time').first()
            student_activity.total_score = latest_record.score if latest_record else 0

        elif activity.retake_method == 'average':
            avg_score = RetakeRecord.objects.filter(student_activity=student_activity).aggregate(Avg('score'))['score__avg']
            student_activity.total_score = avg_score if avg_score is not None else 0

        elif activity.retake_method == 'first':
            first_record = RetakeRecord.objects.filter(student_activity=student_activity).order_by('retake_time').first()
            student_activity.total_score = first_record.score if first_record else 0
...
def activityList(request, subject_id):
    ...
    activities_with_term = Activity.objects.filter(subject=subject, term__semester=current_semester, status=True).exclude(activity_type__name__iexact="Participation")

    # Get activities that do not have any term (copied activities)
    activities_without_term = Activity.objects.filter(subject=subject, term__isnull=True, status=True).exclude(activity_type__name__iexact="Participation")
...
def deleteActivity(request, activity_id):
    ...
    return redirect(reverse('activityList', kwargs={'subject_id': subject_id}))

@login_required
def participation_scores(request, activity_id):
    students = CustomUser.objects.filter(subjectenrollment__subject__activity=activity_id,subjectenrollment__status='enrolled').distinct()
...
class GradeActivityViewCM(View):
    ...
    def post(self, request, activity_id):
        ...
        for key, value in request.POST.items():
            if key.startswith('student_'):  # Expecting input names like 'student_<id>'
                ...
                try:
                    ...
                    if score > activity.max_score:
                        messages.error(request, f"Score for student ID {student_id} exceeds the maximum score of {activity.max_score}.")
                        return redirect('activityListCM', subject_id=activity.subject.id)
        ...
        # Add a success message and redirect after saving all scores
        messages.success(request, "Scores updated successfully.")
        return redirect('viewSubjectModule', pk=module_id)
    ...
    def list(self, request):
        ...
        for component in gradebook_components:
            ...
            if component.is_attendance:
                ...
                print(f"Attendance Percentage for Term {component.term.id}: {attendance_percentage_lookup[component.term.id]}%")

        # Retrieve Term GradeBook Components
        term_percentage_lookup = {}
        term_gradebook_components = TermGradeBookComponents.objects.filter(term__semester=filtered_semester)

        for component in term_gradebook_components:
            term_percentage_lookup[component.term.id] = Decimal(component.percentage)
        ...
        for record in attendance_records:
            ...
            print(f"Student: {student_name}, Term: {term_name}, Attendance Percentage: {attendance_percentage}%")
        ...
        for student_name, data in aggregated_data.items():
            ...
            for term in terms:
                ...
                if grade_calculation_method == "Averaging":
                    ...
                    term_percentage = term_percentage_lookup.get(term.id, Decimal(0)) / 100
            ...
            if has_remedial:
                if passing_grade <= total_final_grade < passing_grade + 0.5:
                    total_final_grade = passing_grade  # Round up to passing if within the 0.5 range
                else:
        ...
        for student_name, data in aggregated_data.items():
            ...
            for term in terms:
                ...
                if attendance_data["max_attendance"] > 0:
                    ...
                    print(f"Student: {student_name}, Term: {term_name}, Attendance Score: {attendance_score}, Percentage Used: {attendance_percentage}%")
...
@login_required
def activities_per_subject(request):
    subjects = Subject.objects.annotate(activity_count=Count('activity'))
    return render(request, 'activity/activities/activities_per_subject.html', {'subjects': subjects})


@login_required
@permission_required('activity.view_activityquestion', raise_exception=True)
def list_activity_questions(request, activity_id):
    """Fetch and display questions for a specific activity, including choices for Multiple Choice questions."""
    activity = get_object_or_404(Activity, id=activity_id)
    # ✅ Retrieve all questions for the given activity
    questions = ActivityQuestion.objects.filter(activity=activity).prefetch_related('choices')
    # ✅ Fetch choices for all retrieved questions
...
def edit_activity_question(request, activity_id, question_id):
    ...
    # ✅ Retrieve left-side and right-side choices separately
    ...
    # ✅ Create zipped list of matching pairs
    matching_pairs = list(zip(left_choices, right_choices[:len(left_choices)]))  # Prevent index errors

    # ✅ Get additional right-side distractors
    ...
    if request.method == "POST":
        ...
        # ✅ Handle Matching Type (Pairs & Distractors)
        elif question.quiz_type.name == 'Matching Type':
            ...
            # ✅ Store Left-Side Choices
            for left in left_texts:
                QuestionChoice.objects.create(question=question, choice_text=left, is_left_side=True)

            # ✅ Store Right-Side Choices (Correct Matches)
            for right in right_texts:
                QuestionChoice.objects.create(question=question, choice_text=right, is_left_side=False)

            # ✅ Store Extra Right-Side Distractors
            for extra in extra_rights:
                QuestionChoice.objects.create(question=question, choice_text=extra, is_left_side=False)

            # ✅ Save Correct Answer as "Left -> Right" Pairs
            question.correct_answer = ", ".join([f"{l} -> {r}" for l, r in zip(left_texts, right_texts)])

        # ✅ Handle True/False Questions
        elif question.quiz_type.name == 'True/False':
            question.correct_answer = request.POST.get('correct_answer', '')

        # ✅ Handle Calculated Numeric or Fill in the Blank
        elif question.quiz_type.name in ['Calculated Numeric', 'Fill in the Blank']:
            question.correct_answer = request.POST.get('correct_answer', '')

        # ✅ Save updated question
        question.save()
        return redirect('list_activity_questions', activity_id=activity_id)

    return render(request, 'activity/question/edit_question.html', {
        'activity': activity,
        'question': question,
        'matching_pairs': matching_pairs,  # ✅ Zipped matching pairs
        'extra_right_choices': extra_right_choices,  # ✅ Extra right-side choices (distractors)
    })


@login_required
@permission_required('activity.delete_activityquestion', raise_exception=True)
def delete_activity_question(request, activity_id, question_id):
    """Delete a specific question from an activity."""
    if request.method == "POST":
        question = get_object_or_404(ActivityQuestion, id=question_id, activity_id=activity_id)
        question.delete()
