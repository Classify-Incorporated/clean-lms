    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if source_subject in target_subjects:
                ...
                return redirect('copyGradeBookComponents')
            
            # Get components to copy from the selected source subject and term
            ...
            # Copy components to each target subject in the current semester
            errors_found = False
            for target_subject in target_subjects:
                for component in components_to_copy:
                    ...
                    if new_percentage + component.percentage > 100:
                        messages.error(request, f"Cannot copy '{component.gradebook_name}'; would exceed 100% in {target_subject}.")
                        errors_found = True
                        break
...
def studentActivityView(request, activity_id):
    ...
    for student_activity in student_activities:
        ...
        print(f"Student: {student.email}, Total Score: {selected_score}")
