...
// Open Attendance Modal for Classroom Mode
function openAttendanceModalCM(subjectId) {
  $('#addActivityLessonModalStandard, #addActivityLessonModalCM').modal('hide').on('hidden.bs.modal', function () {
    $(this).off('hidden.bs.modal'); // Remove event listener to avoid duplicate triggers

    fetch(`/attendance/record_attendanceCM/${subjectId}/`)
      .then((response) => response.text())
      .then((html) => {
        document.getElementById('attendanceModalBody').innerHTML = html;
        $('#attendanceModal').modal('show'); // Properly show modal using Bootstrap
      })
      .catch((error) => console.error('Error loading attendance modal:', error));
  });
}

// Open Attendance Modal for Standard Mode
