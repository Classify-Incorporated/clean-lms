...
$(document).ready(function() {
    // Activate the AdminLTE Sidebar Search Feature
    $('[data-widget="sidebar-search"]').sidebarSearch({
        highlightClass: 'text-primary', // Highlight search results
        notFoundText: 'No element found!' // Custom text for no results
    });
});
$(document).ready(function() {
// Refresh search when sidebar items are loaded
$(document).on('DOMNodeInserted', function() {
    $('[data-widget="sidebar-search"]').sidebarSearch('refresh');
});
});
$(document).ready(function() {
$('[data-widget="sidebar-search"] input').on('keyup', function() {
    var searchText = $(this).val().toLowerCase();
    $('.nav-item a').each(function() {
        var text = $(this).text().toLowerCase();
        if (text.includes(searchText) && searchText !== '') {
            $(this).addClass('text-primary'); // Highlight found item
        } else {
